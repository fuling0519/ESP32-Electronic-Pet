...
- **增加健康資料**：已有六小時疾病觸發、治療、生病後 24 小時清醒死亡門檻及死亡告別畫面；墓碑保存／瀏覽及重養流程仍未完成。疾病、治療、死亡與墓碑紀錄已確定為 MVP 必做。
...
- [ ] 疾病、治療、死亡、墓碑群與重養流程。（六小時疾病觸發、治療、24 小時清醒死亡門檻及死亡告別畫面已做；墓碑保存／瀏覽及重養未做。）
...
3. 清潔操作、疾病觸發、治療、死亡門檻及告別畫面已接上；接著完成墓碑與重養流程，並使疾病進度可保存。
...
- [ ] 隨機結果一經決定即可保存，重開機不重抽。
- [x] 正式寵物主畫面美術規格：標準畫布 64×44，建議原點 `(32, 6)`；所有姿勢與動畫須留在中央 `x=20..108`、`y=0..54` 安全區。44×32 史萊姆只作測試占位，不列入正式種類；共用死亡靈魂使用兩格 30×28 動畫。
...
- [ ] 治療入口、持續疾病警示、死亡畫面、墓碑群瀏覽與新蛋流程。（治療入口、既有病情圖示／狀態頁、核心死亡門檻及死亡告別畫面已接上；墓碑瀏覽與新蛋未完成。）
...
- **2026-09-25**：死亡動畫開始時播放一次蜂鳴器告別音效；依使用者提供的 D4 → C♯4 → D4 → B3（294／277／294／247 Hz）與 500／500／500／1000 ms 音長改寫，每個音後停 50 ms，合計 2.7 秒。沿用既有非阻塞音序，待實機聆聽調整。
- **2026-09-25**：依使用者重畫的 `assets/ghost/GHOST2.png` 更新靈魂：上下兩格各 30×28，保留 X 眼、天使光環及兩種翅膀位置；逐像素轉成兩張單色點陣圖，約每 300 毫秒交替。使用者實機回饋舊位置太居中，因此畫布左緣從 `x=49` 移至 `x=64`（中心約 `x=79`），右側星光避開翅膀。舊圖保留供對照，新版實機效果待驗收。
- **2026-09-25**：加入 B → A 死亡告別流程：測試用史萊姆分三段溶解，升空的共用靈魂改採使用者手繪 `assets/ghost/GHOST.png`，保留白色實心身體與刻意繪製的 X 眼。原圖 60×56、前三格各 30×28；三格 23×27 圖案完全相同，僅位置依序為 `(4,1)`、`(5,1)`、`(4,0)`，故韌體儲存一份點陣圖並用三組位移播放，無額外造型改動。約 3.6 秒後停在含年齡的墓碑畫面。死亡畫面攔截照顧操作，正式寵物主畫面標準畫布定為 64×44。新增 `esp32dev_death_test` 測試環境，開機後直接生病並在 30 秒後死亡，供快速上板驗收；正式環境門檻不變。韌體編譯通過，尚待實機視覺驗收。
