...
- **增加健康資料**：已有六小時疾病觸發與治療；死亡旗標仍沒有自動觸發、墓碑或重養流程。疾病、治療、死亡與墓碑紀錄已確定為 MVP 必做。
...
- `isSick`、`isDead`：初始 false；疾病可由需求持續為 0 觸發，死亡尚未自動觸發。
- `dangerSeconds` 與 `sickAwakeSeconds`：記錄連續危險暴露與生病後清醒時間；目前僅疾病門檻生效，死亡門檻尚未啟用。
...
- [ ] 疾病、治療、死亡、墓碑群與重養流程。（六小時疾病觸發與治療已做；死亡與墓碑未做。）
...
3. 清潔操作、疾病觸發與治療已接上；接著完成死亡、墓碑與重養流程，並使疾病進度可保存。
...
- [ ] 疾病、治療、死亡門檻及等級／經驗邊界集中設定並驗收。（六小時疾病門檻、疾病累計與治療已有原生測試；死亡、等級／經驗規則未做。）
...
- [ ] 治療入口、持續疾病警示、死亡畫面、墓碑群瀏覽與新蛋流程。（治療入口與既有病情圖示／狀態頁已接上，其餘未完成。）
