...
- **增加健康資料**：已有生病、死亡布林旗標，但沒有生病／死亡的觸發或處理規則；疾病、治療、死亡與墓碑紀錄已確定為 MVP 必做。
...
- [x] 主選單含 Feed／Play／Rest／Status，上下循環選取、短按確認、長按返回。
- [x] 主選單 Status 與主畫面狀態卡進入同一個詳細狀態頁；Feed／Play／Rest 目的頁仍為 `Not implemented`。
- [ ] 寵物數值變動時主動通知 UI 重繪；目前以畫面／選項變化觸發 dirty rendering。
...
- `isSick`、`isDead`：初始 false；有 setter，沒有自動觸發機制。
...
尚未有寵物 ID、名字、種類、年齡、蛋／孵化階段、睡眠狀態或時間戳。需要新增的欄位應在相應階段再落實；體力不列入 MVP。
...
主程式目前只更新輸入、UI 與音效，每次開機建立預設寵物；[Save](../src/storage/Save.cpp) 尚無讀寫。[README](../README.md) 已更新為接手入口；[架構筆記](architecture.md) 的「狀態卡僅裝飾」等描述仍落後於工作目錄，實作前以程式與本企劃交叉核對。
...
2. 建立可測試的時間更新介面，讓一般運作與睡醒結算共用狀態計算。
3. 實作需求衰減、清潔操作、疾病／治療／死亡規則及通知；這些已確定納入 MVP。
4. 加入數值變更通知 UI 的方式，避免狀態變了但畫面不更新。
...
- [x] UI 讀取同一隻寵物。
- [ ] 定時更新、數值變化後畫面同步、規則參數。
- [ ] 加入年齡、名字、唯一寵物 ID、疾病進度；體力不加入 MVP。
- [ ] 疾病、治療、死亡門檻及等級／經驗邊界集中設定並驗收。
- [ ] 檢查 0／100 限制及各狀態交界，避免加減溢位。
