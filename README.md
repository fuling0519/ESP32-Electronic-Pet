...
- 清醒時飽食或清潔連續為 0 六小時會生病；Treat 可治療，若需求持續為 0，六小時後可再次生病。此階段還不會因病死亡。
- `Save` 只有初始化占位，尚無保存／載入；每次開機重新建立預設寵物。
- 蛋、孵化、遊戲、一般睡眠、Deep Sleep、死亡處理、墓碑群皆未實作；餵食、清潔及治療是首版可操作流程，尚未實機驗收。
...
- A3 已有清醒狀態更新、暫存年齡、UI 數值同步、六小時疾病門檻及餵食／清潔／治療入口；下一步是死亡與墓碑流程、寵物身分及保存。原先數值下降已獲使用者實機驗收；新增操作頁與病情仍待上板。體力不在 MVP。
