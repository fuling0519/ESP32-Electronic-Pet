...
- `PetData` 目前有飽食度、心情、清潔度、等級、經驗、生病與死亡旗標；清醒運作時已按整秒推進年齡與三項需求，並在可見數值變動後更新畫面。每 10／15／20 分鐘分別降低飽食／清潔／心情 1 點；這是待實機調整的首版預設值。
- 主選單 Feed／Clean／Treat 有操作頁：進入後短按執行，長按返回；Play／Rest 仍是 `Not implemented`。主畫面狀態卡與 Status 都進入同一個兩頁詳細狀態。新操作頁尚未實機驗收。
- 清醒時飽食或清潔連續為 0 六小時會生病；Treat 可治療，若需求持續為 0，六小時後可再次生病。生病後累計 24 小時清醒時間會死亡；死亡後停止照顧與時間推進，播放約 2.7 秒的告別音效、測試用史萊姆溶解及共用的白色 X 眼靈魂升空動畫，最後停在墓碑與年齡畫面。
- `Save` 只有初始化占位，尚無保存／載入；每次開機重新建立預設寵物。
- 蛋、孵化、遊戲、一般睡眠、Deep Sleep、墓碑保存／瀏覽及重養皆未實作；餵食、清潔、治療及死亡告別畫面尚未完整實機驗收。
...
正式寵物的主畫面美術以 **64×44 單色像素**為標準畫布，建議放在 `(32, 6)`；主畫面中央安全區域約為 `x=20..108`、`y=0..54`，包含耳朵、尾巴及動畫位移後都不應超出此區域。安全區理論上可容納約 88×54，但貼近上限會讓左右 HUD 顯得擁擠。史萊姆仍是 44×32 的測試占位圖，不屬於正式寵物種類。死亡動畫共用靈魂以 [GHOST2.png](assets/ghost/GHOST2.png) 為準：圖檔 30×56，從上到下是兩格 30×28 畫面，白色身體、刻意畫的 X 眼、光環和兩種翅膀位置皆逐像素保留。兩格約每 300 毫秒交替，靈魂畫布從 `x=64` 開始（中心 `x≈79`），位於螢幕中央偏右。舊版 [GHOST.png](assets/ghost/GHOST.png) 保留作為歷史素材。

UI 不應直接操作硬體腳位。主迴圈目前以非阻塞方式更新輸入與音效；增加養成計時、保存與睡眠時維持這個方向。修改現有程式前先讀實際檔案，避免依據已過時的文件覆蓋工作目錄變更。

## 下一位開發者／Agent 的注意事項

- A3 已有清醒狀態更新、暫存年齡、UI 數值同步、六小時疾病門檻、24 小時清醒疾病死亡門檻、餵食／清潔／治療入口及死亡告別畫面；下一步是墓碑資料與瀏覽流程、寵物身分及保存。原先數值下降已獲使用者實機驗收；新增操作頁、病情與死亡畫面仍待上板。體力不在 MVP。
